...
class SparseTransInvariantConvolution(SparseModule):
    __constants__ = [
        'stride', 'padding', 'dilation', 'groups', 'bias', 'subm', 'inverse',
        'transposed', 'output_padding', 'fused_bn'
    ]
    ...
    def __init__(self,
                 ...
                 use_hash=False,
                 algo=ops.ConvAlgo.Native):
        super(SparseTransInvariantConvolution, self).__init__()
        assert subm == True
        assert groups == 1
        if not isinstance(kernel_size, (list, tuple)):
            kernel_size = [kernel_size] * ndim
    ...
    def forward(self, input):
        ...
        # 对于convinput来说是False.
        # 说明对于convinput来说, 梯度实际上是只传到filter上的.
        device = features.device
        center_features = input.center_features
        assert center_features is not None 

        indices = input.indices
        spatial_shape = input.spatial_shape
        batch_size = input.batch_size
        if not self.subm:
            assert False, "Only surpport subm with kernelsize > 1 now!"
            if self.transposed:
                out_spatial_shape = ops.get_deconv_output_size(
                    spatial_shape, self.kernel_size, self.stride, self.padding,
                    self.dilation, self.output_padding)
        ...
        if self.fused_bn:
            ...
            if self.subm:
                out_features = Fsp.indice_trans_invariant_conv(features, self.weight,
                                                    center_features,
                                                    indice_pairs.to(device),
                                                    indice_pair_num,
                                                    outids.shape[0], self.algo)
            ...
            if self.bias is not None:
                out_features += self.bias
        # no need prepare the center_fetaures for output 
        out_tensor = spconv.SparseConvTensor(out_features, outids,
                                             out_spatial_shape, batch_size)
        out_tensor.indice_dict = input.indice_dict
        out_tensor.grid = input.grid
...
class SubMTransInvariantConv3d(SparseTransInvariantConvolution):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
                 ...
                 use_hash=False,
                 algo=ops.ConvAlgo.Native):
        super(SubMTransInvariantConv3d, self).__init__(3,
                                         in_channels,
                                         out_channels,
                                         kernel_size,
                                         stride,
