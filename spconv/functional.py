...
class SubMTransInvariantConvFunction(Function):
    """
    NOTE only surpport for subm
    """
    @staticmethod
    def forward(ctx, features, filters, center_features, indice_pairs, indice_pair_num,
                num_activate_out, algo):
        ctx.save_for_backward(indice_pairs, indice_pair_num, features, filters, center_features)
        ctx.algo = algo
        return ops.indice_trans_invariant_conv(features,
                               filters,
                               center_features,
                               indice_pairs,
                               indice_pair_num,
    ...
    def backward(ctx, grad_output):
        """
        No center_bp in this version
        """
        indice_pairs, indice_pair_num, features, filters, center_features = ctx.saved_tensors
        input_bp, filters_bp = ops.indice_trans_invariant_conv_backward(features,
                                                        filters,
                                                        center_features,
                                                        grad_output,
                                                        indice_pairs,
...
class SparseGroupFunction(Function):
    ...
    def forward(ctx, features, indice_pairs, indice_pair_num,
        ...
        ctx.algo = algo
        return ops.indice_group(features,
                               indice_pairs,
                               indice_pair_num,
                               num_activate_out,
                               False,
    ...
    def backward(ctx, grad_output):
        ...
        input_bp = ops.indice_group_backward(features,
                                                        grad_output,
                                                        indice_pairs,
                                                        indice_pair_num,
                                                        False,
...
indice_trans_invariant_conv = SubMTransInvariantConvFunction.apply
