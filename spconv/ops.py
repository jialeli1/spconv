...
def indice_trans_invariant_conv(features,
                filters,
                center_features,
                indice_pairs,
                indice_pair_num,
                ...
                subm=False,
                algo=ConvAlgo.Native.value):
    return torch.ops.spconv.indice_trans_invariant_conv(features, filters, 
                                        center_features,
                                        indice_pairs,
                                        indice_pair_num, num_activate_out,
                                        int(inverse), int(subm), algo)
...
def indice_trans_invariant_conv_backward(features,
                         filters,
                         center_features,
                         out_bp,
                         indice_pairs,
                         ...
                         subm=False,
                         algo=ConvAlgo.Native.value):
    return torch.ops.spconv.indice_trans_invariant_conv_backward(features, filters, 
                                                 center_features,
                                                 out_bp,
                                                 indice_pairs, indice_pair_num,
                                                 int(inverse), int(subm), algo)
...
def indice_group(features,
                ...
                subm=False,
                algo=ConvAlgo.Native.value):
    return torch.ops.spconv.indice_group(features, indice_pairs,
                                        indice_pair_num, num_activate_out,
                                        int(inverse), int(subm), algo)
...
def indice_group_backward(features,
                         ...
                         subm=False,
                         algo=ConvAlgo.Native.value):
    return torch.ops.spconv.indice_group_backward(features, out_bp,
                                                 indice_pairs, indice_pair_num,
                                                 int(inverse), int(subm), algo)
